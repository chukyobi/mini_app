    ...
    try {
        ...
        if (response && response.screen_name === TWITTER_USERNAME) {
            ...
            return res.status(200).json({
                redirectUrl: `http://localhost:4000/auth/authTwitter?token=${encodeURIComponent(token)}&followed=true`,
            });
        }
        else {
            console.log(`User ${decryptedTelegramUserId} failed to follow Twitter account ${TWITTER_USERNAME}`);
            return res.status(200).json({
                redirectUrl: `http://localhost:4000/auth/authTwitter?token=${encodeURIComponent(token)}&followed=false`,
            });
        }
    }
    catch (error) {
        ...
        return res.status(500).json({
            ...
            redirectUrl: `http://localhost:4000/auth/authTwitter?token=${encodeURIComponent(req.query.token)}&followed=false`,
        });
    }
});
